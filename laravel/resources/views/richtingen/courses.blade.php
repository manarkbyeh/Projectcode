...
//filter out the sought schools
$found_school_names = [];
$found_school_locations = [];

for ($i=0; $i < count($arr_school_name); $i++) { 
  if (isset($search_term)) {
    if (stripos($arr_school_name[$i], $search_term) !== false) {
      if (!empty($_SESSION['school_naam'])) {
        foreach ($_SESSION['school_naam'] as $school_naam) {
          if (strpos($arr_school_location[$i], $school_naam) !== false) {
            $new_name = $arr_school_name[$i];
            $new_location = $arr_school_location[$i];

            array_push($found_school_names, $new_name);
            array_push($found_school_locations, $new_location);
          }
        }
      }
      else {
  ...
  elseif (!empty($_SESSION['school_naam'])) {
    foreach ($_SESSION['school_naam'] as $school_naam) {
      if (strpos($arr_school_location[$i], $school_naam) !== false) {
        $new_name = $arr_school_name[$i];
        $new_location = $arr_school_location[$i];
    ...
    <div class="row">
      <div class="school-list">
        ...
        <table class="large_screen">
          <?php if(count($found_school_names) < 1): ?>
            <tr>
              <td>
          ...
          <?php else: ?>
            <?php for ($i=0; $i < count($found_school_names); $i++): ?>
              <tr>
                <td>
                  <?= $found_school_names[$i] ?>
                </td>
                <td>
                  <?= $found_school_locations[$i] ?>
                </td>
              </tr>
            <?php endfor ?>
          <?php endif ?>
